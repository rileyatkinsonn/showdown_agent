...
class Gen9Knowledge:
    ...
    def stab_multiplier(self, attacker_types: List[Any], move_type: Union[Any, str]) -> float:
        """Return 1.5 if any attacker type matches move type (case-insensitive)."""
        m = move_type.name if hasattr(move_type, "name") else str(move_type)
        m = m.upper()
        for at in attacker_types:
            if not at:
                continue
            an = at.name if hasattr(at, "name") else str(at)
            if an.upper() == m:
                return 1.5
        return 1.0
    ...
    def estimate_stat(self, species_id: str, stat: str, level: int = 50, iv: int = 31, ev: int = 84, nature: str = "neutral") -> Optional[int]:
...
class CustomAgent(Player):
    ...
    def _analyze_opponent_threat(self, opponent: Pokemon, my_pokemon: Pokemon) -> float:
        ...
        base = 0.0
        for t in my_pokemon.types:
            if t:
                base = max(base, opponent.damage_multiplier(t))
        threat_level += base
    ...
    def choose_move(self, battle: AbstractBattle):
        ...
        if battle.available_moves and (not self._should_switch_out(battle) or not battle.available_switches):
            ...
            def move_score(m):
                ...
                # Use the enhanced damage calculation
                damage_score = expected_damage_score(active, opponent, m, self.knowledge)
                
                # But also include the original simple logic as backup
                simple_score = (m.base_power * 
                               (1.5 if m.type in active.types else 1.0) *
                               m.accuracy * m.expected_hits *
                               opponent.damage_multiplier(m))
                
                # Use the better of the two approaches
                return max(damage_score, simple_score)
